...
import { entryPassAction } from "@/modules/passes/actions";
import type { EntryPassResult } from "@/modules/passes/service";
...
/** Guest satırı: kişiye özel giriş QR'ı (ilk açılışta oluşturulur, sonra aynı kod gösterilir). */
export function EntryPassButton({ registrationId, guestName }: { registrationId: string; guestName: string }) {
  const ref = useRef<HTMLDialogElement>(null);
  const titleId = useId();
  const [data, setData] = useState<EntryPassResult | null>(null);
  ...
  const load = (reissue: boolean) =>
    startTransition(async () => {
      setError(null);
      const result = await entryPassAction(registrationId, reissue);
      if (result.ok) {
        setData(result.data);
        setConfirming(false);
          ...
          {data && (
            <>
              <div className="mt-5 flex justify-center">
                <QrCode qr={data.qr} size={248} label={`${guestName} giriş QR kodu`} />
              </div>
              <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
                <Badge tone={data.state === "VALID" ? "positive" : data.state === "NOT_YET_VALID" ? "neutral" : "caution"}>
                  {data.stateLabel}
                </Badge>
              ...
              <p className="mt-2 text-xs leading-relaxed text-muted">
                QR yalnızca doğrulama bağlantısı içerir, kişisel bilgi taşımaz. Bağlantı otomatik gönderilmez.
              </p>
